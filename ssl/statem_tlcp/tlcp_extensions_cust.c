 ...
 */
static int custom_ext_add_tlcp_old_cb_wrap(SSL *s, unsigned int ext_type,
...
static int custom_ext_parse_tlcp_old_cb_wrap(SSL *s, unsigned int ext_type,
                                        unsigned int context,
 ...
 */
void custom_ext_init_tlcp(custom_ext_methods *exts)
{
    size_t i;
    custom_ext_method *meth = exts->meths;
...
int custom_ext_parse_tlcp(SSL *s, unsigned int context, unsigned int ext_type,
                     ...
                     size_t chainidx)
{
    ...
    meth = custom_ext_find_tlcp(exts, role, ext_type, NULL);
    /* If not found return success */
    if (!meth)
        return 1;

    /* Check if extension is defined for our protocol. If not, skip */
    if (!extension_is_relevant_tlcp(s, meth->context, context))
        return 1;
    ...
    if (meth->parse_cb(s, ext_type, context, ext_data, ext_size, x, chainidx,
                       &al, meth->parse_arg) <= 0) {
        SSLfatal_tlcp(s, al, SSL_R_BAD_EXTENSION);
        return 0;
    }
 ...
 * buffer.
 */
int custom_ext_add_tlcp(SSL *s, int context, WPACKET *pkt, X509 *x, size_t chainidx,
                   int maxversion)
{
    custom_ext_methods *exts = &s->cert->custext;
    custom_ext_method *meth;
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if (!should_add_extension_tlcp(s, meth->context, context, maxversion))
            continue;
...
int custom_exts_copy_flags_tlcp(custom_ext_methods *dst,
                           const custom_ext_methods *src)
{
    ...
    for (i = 0; i < src->meths_count; i++, methsrc++) {
        custom_ext_method *methdst = custom_ext_find_tlcp(dst, methsrc->role,
...
int custom_exts_copy_tlcp(custom_ext_methods *dst, const custom_ext_methods *src)
{
    size_t i;
    int err = 0;
    ...
    if (src->meths_count > 0) {
        ...
        for (i = 0; i < src->meths_count; i++) {
            ...
            if (methsrc->add_cb != custom_ext_add_tlcp_old_cb_wrap)
                continue;
...
void custom_exts_free_tlcp(custom_ext_methods *exts)
{
    ...
    for (i = 0, meth = exts->meths; i < exts->meths_count; i++, meth++) {
        if (meth->add_cb != custom_ext_add_tlcp_old_cb_wrap)
            continue;
...
int SSL_CTX_has_client_custom_ext_tlcp(const SSL_CTX *ctx, unsigned int ext_type)
{
    return custom_ext_find_tlcp(&ctx->cert->custext, ENDPOINT_CLIENT, ext_type,
                           NULL) != NULL;
}
                                 ...
                                 void *parse_arg)
{
     ...
     */
    if (SSL_extension_supported_tlcp(ext_type)
    ...
    if (custom_ext_find_tlcp(exts, role, ext_type, NULL))
        return 0;
                              ...
                              custom_ext_parse_cb parse_cb, void *parse_arg)
{
    ...
    ret = add_custom_ext_intern(ctx, role, ext_type,
                                context,
                                custom_ext_add_tlcp_old_cb_wrap,
                                custom_ext_free_old_cb_wrap,
                                add_cb_wrap,
                                custom_ext_parse_tlcp_old_cb_wrap,
                                parse_cb_wrap);
...
int SSL_CTX_add_client_custom_ext_tlcp(SSL_CTX *ctx, unsigned int ext_type,
                                  custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  custom_ext_parse_cb parse_cb, void *parse_arg)
...
int SSL_CTX_add_server_custom_ext_tlcp(SSL_CTX *ctx, unsigned int ext_type,
                                  custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  custom_ext_parse_cb parse_cb, void *parse_arg)
...
int SSL_CTX_add_custom_ext_tlcp(SSL_CTX *ctx, unsigned int ext_type,
                           unsigned int context,
                           SSL_custom_ext_add_cb_ex add_cb,
                           SSL_custom_ext_free_cb_ex free_cb,
                           void *add_arg,
...
int SSL_extension_supported_tlcp(unsigned int ext_type)
{
    switch (ext_type) {
