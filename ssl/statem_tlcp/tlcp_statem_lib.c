...
static int ssl_add_cert_to_wpacket_tlcp(SSL *s, WPACKET *pkt, X509 *x);
/*
...
const unsigned char hrrrandom_tlcp[] = {
 ...
 */
int ssl3_do_write_tlcp(SSL *s, int type)
{
    int ret;
    size_t written = 0;
...
int tls_close_construct_packet_tlcp(SSL *s, WPACKET *pkt, int htype)
{
    size_t msglen;
...
int tls_setup_handshake_tlcp(SSL *s)
{
    ...
    if (ssl_get_min_max_version(s, &ver_min, &ver_max, NULL) != 0) {
        SSLfatal_tlcp(s, SSL_AD_PROTOCOL_VERSION, SSL_R_NO_PROTOCOLS_AVAILABLE);
        return 0;
    }
    ok = 0;
    if (s->server) {
        ...
        if (!ok) {
            SSLfatal_data_tlcp(s, SSL_AD_HANDSHAKE_FAILURE,
...
static int get_cert_verify_tbs_data_tlcp(SSL *s, void **hdata, size_t *hdatalen)
{
    size_t retlen;
    long retlen_l;
...
int tls_construct_cert_verify_tlcp(SSL *s, WPACKET *pkt)
{
    EVP_PKEY *pkey = NULL;
    ...
    if (!get_cert_verify_tbs_data_tlcp(s, &hdata, &hdatalen)) {
...
MSG_PROCESS_RETURN tls_process_cert_verify_tlcp(SSL *s, PACKET *pkt)
{
    EVP_PKEY *pkey = NULL;
    ...
    /* For TLCP server, s->session->peer stores the client signing certificate */
    peer = s->session->peer;
...
int tls_construct_finished_tlcp(SSL *s, WPACKET *pkt)
{
...
/*
 * ssl3_take_mac_tlcp calculates the Finished MAC for the handshakes messages seen
 * to far.
 */
...
MSG_PROCESS_RETURN tls_process_change_cipher_spec_tlcp(SSL *s, PACKET *pkt)
{
    size_t remain;
     ...
     */
    if (remain != 0) {
        SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_CHANGE_CIPHER_SPEC);
        return MSG_PROCESS_ERROR;
    }

    /* Check we have a cipher to change to */
    if (s->s3.tmp.new_cipher == NULL) {
        SSLfatal_tlcp(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_R_CCS_RECEIVED_EARLY);
        return MSG_PROCESS_ERROR;
    }
...
MSG_PROCESS_RETURN tls_process_finished_tlcp(SSL *s, PACKET *pkt)
{
    size_t md_len;
    ...
    if (!s->s3.change_cipher_spec) {
        SSLfatal_tlcp(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_R_GOT_A_FIN_BEFORE_A_CCS);
        return MSG_PROCESS_ERROR;
    }
    ...
    if (md_len != PACKET_remaining(pkt)) {
        SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_DIGEST_LENGTH);
        return MSG_PROCESS_ERROR;
    }

    if (CRYPTO_memcmp(PACKET_data(pkt), s->s3.tmp.peer_finish_md,
                      md_len) != 0) {
        SSLfatal_tlcp(s, SSL_AD_DECRYPT_ERROR, SSL_R_DIGEST_CHECK_FAILED);
        return MSG_PROCESS_ERROR;
    }
...
int tls_construct_change_cipher_spec_tlcp(SSL *s, WPACKET *pkt)
{
...
static int ssl_add_cert_to_wpacket_tlcp(SSL *s, WPACKET *pkt, X509 *x)
{
    ...
    len = i2d_X509(x, NULL);
    if (len < 0) {
        SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, ERR_R_BUF_LIB);
        return 0;
    }
...
static int ssl_add_cert_chain_tlcp(SSL *s, WPACKET *pkt,
                                   CERT_PKEY *a_cpk, CERT_PKEY *k_cpk)
{
    ...
    if (chain_store != NULL) {
        ...
        if (!X509_STORE_CTX_init(xs_ctx, chain_store,
            ...
            SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, ERR_R_X509_LIB);
            return 0;
        }
        /*
...
#endif
            X509_STORE_CTX_free(xs_ctx);
            SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, i);
            return 0;
        }

        /* add sign certificate */
        if (!ssl_add_cert_to_wpacket_tlcp(s, pkt, a_cpk->x509)) {
            /* SSLfatal_tlcp() already called */
            X509_STORE_CTX_free(xs_ctx);
            return 0;
        }
        ...
        if (!ssl_add_cert_to_wpacket_tlcp(s, pkt, k_cpk->x509)) {
            /* SSLfatal_tlcp() already called */
            X509_STORE_CTX_free(xs_ctx);
            return 0;
        }
        ...
        for (i = 1; i < chain_count; i++) {
            ...
            if (!ssl_add_cert_to_wpacket_tlcp(s, pkt, x)) {
                /* SSLfatal_tlcp() already called */
                X509_STORE_CTX_free(xs_ctx);
                return 0;
            }
...
unsigned long ssl3_output_cert_chain_tlcp(SSL *s, WPACKET *pkt,
                                          CERT_PKEY *a_cpk,
                                          CERT_PKEY *k_cpk)
{
    ...
    if (!ssl_add_cert_chain_tlcp(s, pkt, a_cpk, k_cpk))
        return 0;
 ...
 * freed up as well.
 */
WORK_STATE tls_finish_handshake_tlcp(SSL *s, ossl_unused WORK_STATE wst,
                                     int clearbufs, int stop)
{
    ...
    if (cleanuphand) {
        ...
        if (s->server) {
            ...
            s->handshake_func = ossl_statem_accept_tlcp;
        } else {
            ...
            s->handshake_func = ossl_statem_connect_tlcp;
            ssl_tsan_counter(s->session_ctx,
    ...
    ossl_statem_set_in_init_tlcp(s, 0);
    ...
    if (!stop) {
        ...
        ossl_statem_set_in_init_tlcp(s, 1);
...
int tls_get_message_header_tlcp(SSL *s, int *mt)
{
        ...
        while (s->init_num < SSL3_HM_HEADER_LENGTH) {
            ...
            if (recvd_type == SSL3_RT_CHANGE_CIPHER_SPEC) {
                ...
                if (s->init_num != 0 || readbytes != 1 || p[0] != SSL3_MT_CCS) {
                    SSLfatal_tlcp(s, SSL_AD_UNEXPECTED_MESSAGE,
...
int tls_get_message_body_tlcp(SSL *s, size_t *len)
{
    size_t n, readbytes;
    unsigned char *p;
    int i;
     ...
     */
    if (*(s->init_buf->data) == SSL3_MT_FINISHED && !ssl3_take_mac_tlcp(s)) {
        /* SSLfatal_tlcp() already called */
        *len = 0;
        return 0;
    }
...
int ssl_x509err2alert_tlcp(int x509err)
{
...
int ssl_allow_compression_tlcp(SSL *s)
{
...
/*
 * ssl_version_supported_tlcp - Check that the specified `version` is supported by
 * `SSL *` instance
 *
 * @s: The SSL handle for the candidate method
 ...
 */
int ssl_version_supported_tlcp(const SSL *s, int version, const SSL_METHOD **meth)
{
...
/*
 * ssl_check_version_downgrade_tlcp - In response to RFC7507 SCSV version
 ...
 */
int ssl_check_version_downgrade_tlcp(SSL *s)
{
    const version_info *vent;
    const version_info *table;
...
/*
 * ssl_set_version_bound_tlcp - set an upper or lower bound on the supported (D)TLS
 ...
 */
int ssl_set_version_bound_tlcp(int method_version, int version, int *bound)
{
    int valid_tls;
    ...
    valid_tls = version >= TLCP_VERSION && version <= TLS_MAX_VERSION_INTERNAL;
...
static void check_for_downgrade(SSL *s, int vers, DOWNGRADE *dgrd)
{
    if (vers == TLS1_2_VERSION
            && ssl_version_supported_tlcp(s, TLS1_3_VERSION, NULL)) {
        *dgrd = DOWNGRADE_TO_1_2;
                ...
                */
            && ssl_version_supported_tlcp(s, TLS1_2_VERSION, NULL)) {
...
/*
 * ssl_choose_server_version_tlcp - Choose server (D)TLS version.  Called when the
 ...
 */
int ssl_choose_server_version_tlcp(SSL *s, CLIENTHELLO_MSG *hello, DOWNGRADE *dgrd)
{
    /*-
...
/*
 * ssl_choose_client_version_tlcp - Choose client (D)TLS version.  Called when the
 ...
 */
int ssl_choose_client_version_tlcp(SSL *s, int version, RAW_EXTENSION *extensions)
{
    const version_info *vent;
    const version_info *table;
    ...
    if (s->hello_retry_request != SSL_HRR_NONE
        ...
        s->version = origv;
        SSLfatal_tlcp(s, SSL_AD_PROTOCOL_VERSION, SSL_R_WRONG_SSL_VERSION);
        return 0;
    }
    ...
    ret = ssl_get_min_max_version_tlcp(s, &ver_min, &ver_max, &real_max);
    if (ret != 0) {
        s->version = origv;
        SSLfatal_tlcp(s, SSL_AD_PROTOCOL_VERSION, ret);
        return 0;
    }
    if (s->version < ver_min) {
        s->version = origv;
        SSLfatal_tlcp(s, SSL_AD_PROTOCOL_VERSION, SSL_R_UNSUPPORTED_PROTOCOL);
        return 0;
...
/*
 * ssl_get_min_max_version_tlcp - get minimum and maximum protocol version
 ...
 */
int ssl_get_min_max_version_tlcp(const SSL *s, int *min_version, int *max_version,
                            int *real_max)
{
...
/*
 * ssl_set_client_hello_version_tlcp - Work out what version we should be using for
 ...
 * Returns 0 on success or an SSL error reason number on failure.
 */
int ssl_set_client_hello_version_tlcp(SSL *s)
{
    ...
    ret = ssl_get_min_max_version_tlcp(s, &ver_min, &ver_max, NULL);
...
int parse_ca_names_tlcp(SSL *s, PACKET *pkt)
{
    ...
    while (PACKET_remaining(&cadns)) {
        ...
        if (namebytes != (namestart + name_len)) {
            SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_CA_DN_LENGTH_MISMATCH);
            goto err;
        }
...
int ssl_derive_tlcp(SSL *s, EVP_PKEY *privkey, EVP_PKEY *pubkey, int gensecret)
{
     ...
     * XXX:
     *
     * For TLCP server side, s->session->peer stores the client signing
     ...
     *
     * For client side of TLCP, the peer is an reference of the first element
    ...
    /* for TLCP, server is initiator(Z_A), client is responder(Z_B) */
...
#define PEEK_HEADER_LENGTH 3
     ...
     * we can easily determine if version is TLCP
     */
    if (s->version == TLCP1_1_VERSION)
        return 1;
    ...
    if (is_server) {
         ...
         */
        if (s->clienthello) {
            if (s->clienthello->legacy_version == TLCP1_1_VERSION)
                return 1;
            else
                return 0;
        }
        ...
        if (SSL_IS_FIRST_HANDSHAKE(s) && SSL_in_before(s)) {
            ...
            if (BIO_method_type(s->rbio) == BIO_TYPE_MEM) {
                ...
                if (ret <= 0) {
                    /* TLCP only support socket communication */
