...
EXT_RETURN tls_construct_ctos_server_name_tlcp(SSL *s, WPACKET *pkt,
...
EXT_RETURN tls_construct_ctos_maxfragmentlen_tlcp(SSL *s, WPACKET *pkt,
                                             unsigned int context, X509 *x,
                                             size_t chainidx)
{
...
EXT_RETURN tls_construct_ctos_use_srtp_tlcp(SSL *s, WPACKET *pkt,
                                       unsigned int context, X509 *x,
                                       size_t chainidx)
{
                                        ...
                                        size_t chainidx)
{
    ...
    if (curve_id == 0) {
        SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, SSL_R_NO_SUITABLE_KEY_SHARE);
        return EXT_RETURN_FAIL;
    }
                                         ...
                                         size_t chainidx)
{
    ...
    if (s->psk_use_session_cb != NULL
        ...
        SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, SSL_R_BAD_PSK);
        return EXT_RETURN_FAIL;
    }
    ...
    if ((s->ext.alpn == NULL && edsess->ext.alpn_selected != NULL)) {
        SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, SSL_R_INCONSISTENT_EARLY_DATA_ALPN);
        return EXT_RETURN_FAIL;
    }
                                  ...
                                  X509 *x, size_t chainidx)
{
 ...
 dopsksess:
    ...
    if (dores
            && tls_psk_do_binder_tlcp(s, mdres, msgstart, binderoffset, NULL,
    ...
    if (s->psksession != NULL
            && tls_psk_do_binder_tlcp(s, mdpsk, msgstart, binderoffset, NULL,
...
EXT_RETURN tls_construct_ctos_post_handshake_auth_tlcp(SSL *s, WPACKET *pkt,
                                  ...
                                  X509 *x, size_t chainidx)
{
    if (s->ext.session_ticket_cb != NULL &&
        ...
        SSLfatal_tlcp(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_BAD_EXTENSION);
        return 0;
    }

    if (!tls_use_ticket(s)) {
        SSLfatal_tlcp(s, SSL_AD_UNSUPPORTED_EXTENSION, SSL_R_BAD_EXTENSION);
        return 0;
    }
    if (PACKET_remaining(pkt) > 0) {
        SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_EXTENSION);
                       ...
                       size_t chainidx)
{
    ...
    if (s->ct_validation_callback != NULL) {
         ...
         */
        if (custom_ext_find_tlcp(&s->cert->custext, role,
                            TLSEXT_TYPE_signed_certificate_timestamp,
                            NULL) == NULL) {
            SSLfatal_tlcp(s, TLS1_AD_UNSUPPORTED_EXTENSION, SSL_R_BAD_EXTENSION);
            return 0;
        }

        if (!custom_ext_parse_tlcp(s, context,
                             TLSEXT_TYPE_signed_certificate_timestamp,
                            ...
                            size_t chainidx)
{
    ...
    if (!PACKET_get_net_2(pkt, &ct) || ct != 2
        ...
        SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR,
    ...
    if (mki != 0) {
        ...
        SSLfatal_tlcp(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_SRTP_MKI_VALUE);
        return 0;
    }
    ...
    if (clnt == NULL) {
        SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_NO_SRTP_PROFILES);
        return 0;
    }
                             ...
                             size_t chainidx)
{
    ...
    if ((context & SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST) != 0) {
        ...
        if (group_id == s->s3.group_id) {
            SSLfatal_tlcp(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
            return 0;
        }
    ...
    if (!ginf->is_kem) {
        ...
        if (skey == NULL || EVP_PKEY_copy_parameters(skey, ckey) <= 0) {
            SSLfatal_tlcp(s, SSL_AD_INTERNAL_ERROR, SSL_R_COPY_PARAMETERS_FAILED);
            EVP_PKEY_free(skey);
            return 0;
        }
                              ...
                              X509 *x, size_t chainidx)
{
    if (context == SSL_EXT_TLS1_3_NEW_SESSION_TICKET) {
        ...
        if (!PACKET_get_net_4(pkt, &max_early_data)
                || PACKET_remaining(pkt) != 0) {
            SSLfatal_tlcp(s, SSL_AD_DECODE_ERROR, SSL_R_INVALID_MAX_EARLY_DATA);
            return 0;
        }
                       ...
                       size_t chainidx)
{
    ...
    if (identity >= (unsigned int)s->ext.tick_identity) {
        SSLfatal_tlcp(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_PSK_IDENTITY);
        return 0;
    }
