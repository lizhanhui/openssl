...
static int protocol_from_string(const char *value)
{
    ...
    static const struct protocol_versions versions[] = {
        ...
        {"TLCP", TLCP_VERSION},
        {"MIN_VERSION_WITH_TLCP", MIN_VERSION_WITH_TLCP}
#endif
    };
    size_t i;
...
#ifndef OPENSSL_NO_TLCP
static int cmd_Enable_tlcp(SSL_CONF_CTX *cctx, const char *value)
{
    if (strcmp(value, "on") == 0) {
        if (cctx->ctx)
            SSL_CTX_enable_tlcp(cctx->ctx);
        if (cctx->ssl)
            SSL_enable_tlcp(cctx->ssl);
    } else {
        if (cctx->ctx)
            SSL_CTX_disable_tlcp(cctx->ctx);
        if (cctx->ssl)
            SSL_disable_tlcp(cctx->ssl);
    }
    return 1;
}
#endif
...
#ifndef OPENSSL_NO_TLCP
    SSL_CONF_CMD_STRING(Enable_tlcp, "enable_tlcp", 0),
