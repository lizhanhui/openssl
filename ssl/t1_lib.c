...
int tls1_set_peer_legacy_sigalg(SSL *s, const EVP_PKEY *pkey)
{
    ...
    if (SSL_IS_TLCP(s)) {
        if (EVP_PKEY_is_a(pkey, "SM2"))
            s->s3.tmp.peer_sigalg = &tlcp_sm2_sigalg;
        else if (EVP_PKEY_is_a(pkey, "RSA"))
            s->s3.tmp.peer_sigalg = &tlcp_rsa_sigalg;
        else
            return 0;
...
#ifndef OPENSSL_NO_TLCP
        /*
         * TLCP cipher can only use in TLCP
         */
        if (min_tls == TLCP_VERSION)
            if (s->s3.tmp.max_ver != TLCP_VERSION)
                return 1;
#endif
        /*
...
#ifndef OPENSSL_NO_TLCP
int tls_choose_sigalg_tlcp(SSL *s, int fatalerrs)
{
    const SIGALG_LOOKUP *lu = NULL;
    uint32_t kalg;
