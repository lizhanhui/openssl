...
#ifndef OPENSSL_NO_TLCP
int SSL_is_tlcp(const SSL *s)
{
    return SSL_IS_TLCP(s) ? 1 : 0;
}
#endif
...
SSL_cert_cb_fn SSL_CTX_get_cert_cb(SSL_CTX *c)
{
    return ssl_cert_get_cert_cb(c->cert);
}

void *SSL_CTX_get_cert_cb_arg(SSL_CTX *c)
{
...
SSL_cert_cb_fn SSL_get_cert_cb(SSL *s)
{
    return ssl_cert_get_cert_cb(s->cert);
}

void *SSL_get_cert_cb_arg(SSL *s)
{
    return ssl_cert_get_cert_cb_arg(s->cert);
}
...
void ssl_set_masks(SSL *s)
{
    ...
    int tlcp_rsa_enc, tlcp_rsa_sign;
#endif
    ...
    tlcp_rsa_sign = pvalid[SSL_PKEY_RSA_SIGN] & CERT_PKEY_VALID;
    tlcp_rsa_enc = pvalid[SSL_PKEY_RSA_ENC] & CERT_PKEY_VALID;
#endif
    mask_k = 0;
    mask_a = 0;
...
int SSL_get_error(const SSL *s, int i)
{
    ...
    if (SSL_want_read(s)) {
#ifndef OPENSSL_NO_TLCP
        if (s->enable_tlcp == 1 && SSL_IS_FIRST_HANDSHAKE(s))
...
SSL_CTX *SSL_set_SESSION_CTX(SSL *ssl, SSL_CTX *ctx)
{
    ...
    SSL_CTX_up_ref(ctx);
    SSL_CTX_free(ssl->session_ctx);     /* decrement reference count */
    ssl->session_ctx = ctx;
...
int SSL_client_hello_get1_extensions(SSL *s, int **out, size_t *outlen)
{
    int *exts, i = 0;
    size_t num = 0;

    PACKET extensions = s->clienthello->extensions;
    ...
    extensions = s->clienthello->extensions;
...
#ifndef OPENSSL_NO_TLCP
void SSL_CTX_enable_tlcp(SSL_CTX *ctx)
{
    ctx->enable_tlcp = 1;
}

void SSL_CTX_disable_tlcp(SSL_CTX *ctx)
{
    ctx->enable_tlcp = 0;
}
...
int SSL_SESSION_get_ref(SSL_SESSION *sess)
{
    if(sess != NULL)
...
X509 *SSL_get_use_certificate(const SSL *s)
{
    CERT *c = s->cert;
    int idx;
...
#ifndef OPENSSL_NO_TLCP
X509 *SSL_get_sign_certificate_tlcp(const SSL *s)
{
    if (s != NULL && s->s3.tmp.sign_cert != NULL)
        return s->s3.tmp.sign_cert->x509;

    return NULL;
}

X509 *SSL_get_enc_certificate_tlcp(const SSL *s)
{
    if (s != NULL && s->s3.tmp.enc_cert != NULL)
...
/*
 *  SSL_get0_alpn_proposed gets the proposed ALPN protocol (if any) from
 *  |ssl|. On return it sets |*data| to point to |*len| bytes of protocol name
 *  (not including the leading length-prefix byte). If the server didn't
 *  respond with a negotiated protocol then |*len| will be zero.
 */
void SSL_get0_alpn_proposed(const SSL *ssl, const unsigned char **data,
                                unsigned *len)
{
...
void SSL_CTX_certs_clear(SSL_CTX *ctx)
{
    ssl_cert_clear_certs(ctx->cert);
}

int SSL_check_tlsext_status(SSL *s)
{
...
/* TODO: condition opt */
int SSL_get_master_key(SSL *s, unsigned char **master_key,
                           int *master_key_len)
{
...
void SSL_debug(SSL *s, unsigned char *str, int len)
{
    int i;
    if (!str)
        return;
