...
#  ifndef OPENSSL_NO_SM2
    TLCP_TXT_SM2DHE_WITH_SM4_SM3,
    TLCP_TXT_SM2_WITH_SM4_SM3,
    TLCP_TXT_ECDHE_SM2_SM4_CBC_SM3,
    TLCP_TXT_ECDHE_SM2_SM4_GCM_SM3,
    TLCP_TXT_ECC_SM2_SM4_CBC_SM3,
    TLCP_TXT_ECC_SM2_SM4_GCM_SM3,
    TLCP_TXT_SM2DHE_WITH_SM4_SM3":"TLCP_TXT_ECDHE_SM2_SM4_CBC_SM3,
    TLCP_TXT_ECDHE_SM2_SM4_CBC_SM3":"TLCP_TXT_ECDHE_SM2_SM4_GCM_SM3,
    TLCP_TXT_ECDHE_SM2_SM4_CBC_SM3":"TLCP_TXT_ECC_SM2_SM4_CBC_SM3,
    TLCP_TXT_ECDHE_SM2_SM4_CBC_SM3":"TLCP_TXT_RSA_SM4_CBC_SM3,
#  endif /* OPENSSL_NO_SM2 */
    TLCP_TXT_RSA_SM4_CBC_SM3,
    TLCP_TXT_RSA_SM4_GCM_SM3,
# endif /* OPENSSL_NO_SM3 */
    TLCP_TXT_RSA_SM4_CBC_SHA256,
    TLCP_TXT_RSA_SM4_GCM_SHA256,
#endif /* OPENSSL_NO_SM4 */
    NULL,   /* suppress compile error: zero or negative size array */
};

static int test_tlcp_ctx_set_cipher_list(int i)
{
    int           ret = 1;
    SSL_CTX      *ctx = NULL;
    ...
    if (!TEST_true(ctx->enable_tlcp == 1))
        goto err;
...
static int test_tlcp_ssl_set_cipher_list(int i)
{
    int           ret = 1;
    SSL_CTX      *ctx = NULL;
    SSL          *ssl = NULL;
...
static int test_tlcp_ctx_set_cert_pkey_file_api(int i)
{
    int ret = 1;
...
# endif
    ...
    if (!TEST_true(ctx->enable_tlcp == 1))
        goto err;
    SSL_CTX_disable_tlcp(ctx);
    if (!TEST_true(ctx->enable_tlcp == 0))
        goto err;
...
static int test_tlcp_ssl_set_cert_pkey_file_api(int i)
{
    int           ret = 1;
    const char   *sign_cert_file = NULL;
    const char   *sign_key_file = NULL;
...
# endif
    ...
    if (!TEST_true(SSL_is_tlcp(ssl) == 1))
        goto err;

    SSL_enable_tlcp(ssl);
    if (!TEST_true(ssl->enable_tlcp == 1))
        goto err;
    SSL_disable_tlcp(ssl);
    if (!TEST_true(ssl->enable_tlcp == 0))
        goto err;
...
static int test_tlcp_ctx_set_cert_pkey_api(int i)
{
    int           ret = 1;
    SSL_CTX      *ctx = NULL;
    X509         *sign_cert = NULL;
...
static int test_tlcp_ssl_set_cert_pkey_api(int i)
{
    int           ret = 1;
    const char   *sign_cert_file = NULL;
    const char   *sign_key_file = NULL;
...
static int test_tlcp_method_api(void)
{
    int ret = 1;
    ...
    ret = 0;
    meth = TLCP_method();
    if (!TEST_true(meth->version == TLCP_VERSION))
        goto err;
    if (!TEST_true(meth->flags == SSL_METHOD_NO_SUITEB))
        goto err;
    if (!TEST_true(meth->mask == SSL_OP_NO_TLCP))
        goto err;

    meth = TLCP_server_method();
    if (!TEST_true(meth->version == TLCP_VERSION))
        goto err;
    if (!TEST_true(meth->flags == SSL_METHOD_NO_SUITEB))
        goto err;
...
int setup_tests(void)
{
    if (!TEST_ptr(sm2_sign_cert_file = test_get_argument(0))
        ...
        TEST_note("usage: ssl_tlcp_api_test cert.pem|key.pem");
        return 0;
    }
    ADD_ALL_TESTS(test_tlcp_ctx_set_cert_pkey_file_api, 2);
    ADD_ALL_TESTS(test_tlcp_ctx_set_cert_pkey_api, 2);
    ADD_ALL_TESTS(test_tlcp_ssl_set_cert_pkey_file_api, 2);
    ADD_ALL_TESTS(test_tlcp_ssl_set_cert_pkey_api, 2);
    ADD_TEST(test_tlcp_method_api);

    ADD_ALL_TESTS(test_tlcp_ctx_set_cipher_list, OSSL_NELEM(cipher_list) - 1);
    ADD_ALL_TESTS(test_tlcp_ssl_set_cipher_list, OSSL_NELEM(cipher_list) - 1);
    return 1;
}