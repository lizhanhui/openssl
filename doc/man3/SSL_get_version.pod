...
SSL_is_tlcp, SSL_enable_tlcp, SSL_disable_tlcp,
SSL_CTX_enable_tlcp, SSL_CTX_disable_tlcp - get the
...
SSL_is_tlcp() returns one if the connection is using TLCP, zero if not.

SSL_enable_tlcp() and SSL_CTX_enable_tlcp() enable TLCP protocol either for
a given SSL connection or for all connections that share the given SSL context,
respectively. SSL_disable_tlcp and SSL_CTX_disable_tlcp() turn off TLCP
protocol.
